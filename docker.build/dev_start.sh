...
###############################################################################
DOCKER_BUILD_ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd -P)"
source "${DOCKER_BUILD_ROOT_DIR}/scripts/apollo.bashrc"

VERSION_X86_64="dev-x86_64-18.04-20200602_2144"
VERSION_AARCH64="dev-aarch64-18.04-20200525_0656"
VERSION_LOCAL_DEV="local_story_dev"
DEV_CONTAINER="story_dev_${USER}"
DEV_INSIDE="in_dev_docker"

DOCKER_REPO="storypku/storydev"
...
# <dev|dev>-<arch>-<ubuntu-release>-<timestamp>
...
function setup_devices_and_mount_volumes() {
  ...
  local volumes
  volumes="-v ${DOCKER_BUILD_ROOT_DIR}:/apollo"
...
function remove_existing_dev_container() {
  if docker ps -a --format '{{.Names}}' | grep -q "${DEV_CONTAINER}"; then
    info "Removing existing dev container ${DEV_CONTAINER}"
    docker stop "${DEV_CONTAINER}" > /dev/null
    docker rm -v -f "${DEV_CONTAINER}" > /dev/null
  fi
}
...
function docker_pull_if_needed() {
  ...
  if [[ ${use_local_image} -eq 1 ]]; then
    info "Start dev container based on local image: ${image}"
...
function after_run_setup() {
  if [[ "${USER}" != "root" ]]; then
    docker exec -u root "${DEV_CONTAINER}" \
