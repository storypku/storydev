  ...
  ws.set_option(
      websocket::stream_base::decorator([](websocket::response_type& res) {
        res.set(http::field::server,
                std::string(BOOST_BEAST_VERSION_STRING) + "-Sync");
      }));
...
void do_sync_listen(net::io_context& ioc, tcp::endpoint endpoint) {
  beast::error_code ec;
...
class async_session : public std::enable_shared_from_this<async_session> {
 ...
 public:
  ...
  explicit async_session(tcp::socket&& socket) : ws_(std::move(socket)) {
    setup_stream(ws_);
  }
  ...
  void run() {
    ...
    ws_.set_option(
        websocket::stream_base::decorator([](websocket::response_type& res) {
          res.set(http::field::server,
                  std::string(BOOST_BEAST_VERSION_STRING) + "-Async");
        }));

    // Accept the websocket handshake
    ws_.async_accept(beast::bind_front_handler(&async_session::on_accept,
                                               shared_from_this()));
  }
  ...
  void do_read() {
    ...
    ws_.async_read(buffer_, beast::bind_front_handler(&async_session::on_read,
                                                      shared_from_this()));
  }
  ...
  void on_read(beast::error_code ec, std::size_t bytes_transferred) {
    ...
    ws_.async_write(buffer_.data(),
                    beast::bind_front_handler(&async_session::on_write,
                                              shared_from_this()));
  }
...
class async_listener : public std::enable_shared_from_this<async_listener> {
 ...
 public:
  async_listener(net::io_context& ioc, tcp::endpoint endpoint)
 ...
 private:
  void do_accept() {
    ...
    acceptor_.async_accept(net::make_strand(ioc_),
                           beast::bind_front_handler(&async_listener::on_accept,
                                                     shared_from_this()));
  }
...
void do_coro_session(websocket::stream<beast::tcp_stream>& ws,
  ...
  ws.set_option(
      websocket::stream_base::decorator([](websocket::response_type& res) {
        res.set(http::field::server,
                std::string(BOOST_BEAST_VERSION_STRING) + "-Fiber");
      }));
...
void do_coro_listen(net::io_context& ioc, tcp::endpoint endpoint,
                    net::yield_context yield) {
  beast::error_code ec;
...
int main(int argc, char* argv[]) {
  ...
  std::thread(
      beast::bind_front_handler(
          &do_sync_listen, std::ref(ioc),
          tcp::endpoint{address, static_cast<unsigned short>(port + 0u)}))
      .detach();

  // Create async port
  std::make_shared<async_listener>(
      ioc, tcp::endpoint{address, static_cast<unsigned short>(port + 1u)})
      ->run();

  // Create coro port
  boost::asio::spawn(
      ioc,
      std::bind(&do_coro_listen, std::ref(ioc),
                tcp::endpoint{address, static_cast<unsigned short>(port + 2u)},
                std::placeholders::_1));
