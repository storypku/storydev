...
class http_worker {
 ...
 private:
  ...
  void send_file(beast::string_view target) {
    ...
    std::string full_path = path_cat(doc_root_, target);
    if (full_path.back() == '/') full_path.append("index.html");

    http::file_body::value_type file;
    beast::error_code ec;
    file.open(full_path.c_str(), beast::file_mode::scan, ec);
